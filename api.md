...
```http
GET /question/list?activity_id=1&page=1&size=10&keyword=图书馆
```
...
```http
GET /answer/list?question_id=1001&page=1&size=10
```
...
```http
GET /comment/list?question_id=1001&page=1&size=10
```
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| target_type | string | 是 | 目标类型：`question` / `answer` / `comment` |
