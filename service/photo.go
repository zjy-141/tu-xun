...
func (info *PhotoSvc) Create(params PhotoCreateParams) (resp ResponseIS, err error) {
	...
	if !activity.IsActive ||
		(activity.StartTime != nil && now.Before(*activity.StartTime)) ||
		(activity.EndTime != nil && !now.Before(*activity.EndTime)) {
		tx.Rollback()
...
func (info *PhotoSvc) List(params PhotoListParams, userID int64) (resp PhotoCardPage, err error) {
	...
	if params.ActivityStatus != "" {
		...
		case "active":
			activitySubQuery = activitySubQuery.Where("is_active = ? AND start_time <= ? AND end_time > ?", true, now, now)
		case "ended":
			activitySubQuery = activitySubQuery.Where("end_time <= ?", now)
		}
